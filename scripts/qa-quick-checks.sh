...
RUN_VPS_AUTH_CHECK_NORM="$(printf '%s' "$RUN_VPS_AUTH_CHECK" | tr '[:upper:]' '[:lower:]')"
...

{
  ...
  if [[ "$RUN_VPS_AUTH_CHECK_NORM" == "true" ]]; then
    echo "[qa-quick-checks] RUN_VPS_AUTH_CHECK=true; running bridge auth preflight"
    (cd "$ROOT_DIR" && bash ./scripts/qa-bridge-auth-check.sh "$RUN_ID")
    echo
  else
    echo "[qa-quick-checks] RUN_VPS_AUTH_CHECK=false; skipping bridge auth preflight"
    echo
  fi
