...
const RECALL_API_BASE = String(process.env.RECALL_API_BASE || '').replace(/\/+$/, '');
const RECALL_BOTS_ENDPOINT = RECALL_API_BASE ? `${RECALL_API_BASE}/api/v1/bot` : '';
...
function getRecallConfigIssue() {
  if (!RECALL_API_BASE) {
    return 'RECALL_API_BASE is missing. Set it to your Recall workspace region endpoint (for example: https://us-east-1.recall.ai).';
  }
  if (!RECALL_API_KEY) {
    return 'RECALL_API_KEY is missing. Set it in the bridge environment.';
  }
  return '';
}
...
app.get('/health', (req, res) => {
  ...
  res.json({
    ...
    bridge_auth: {
      enabled: BRIDGE_AUTH_ENABLED
    },
    recall: {
      api_base: RECALL_API_BASE || '',
      api_base_configured: Boolean(RECALL_API_BASE)
    },
    hook: {
...
app.post('/launch', requireBridgeAuth, async (req, res) => {
  ...
  const recallIssue = getRecallConfigIssue();
  if (recallIssue) {
    return res.status(503).json({
      error: 'Bridge Recall configuration is incomplete',
      code: 'recall_config_incomplete',
      remediation: recallIssue
    });
  }
...
app.listen(PORT, HOST, () => {
  ...
  console.log(`[Recall] api_base=${RECALL_API_BASE || 'missing'}`);
  ...
  const recallIssue = getRecallConfigIssue();
  if (recallIssue) {
    console.warn(`[Recall] ${recallIssue}`);
  }
  if (!OPENCLAW_HOOK_TOKEN) {
